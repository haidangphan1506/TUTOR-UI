...
| GET    | `/classes/:id/watches`   | JWT  | success                | —                                                                      | `{ class, recentSession, schedules, students }`                                                                                                     | All         |              |
| GET    | `/classes/:id/watches`   | JWT  | 400/404                | —                                                                      | UUID errors • `"Class not found"` (also when caller has no access to the class)                                                                     | All         |              |
...
Notes: `/classes/:id/watches` — aggregated STUDENT/PARENT "watch" overview added 2026-08-29 (was a
404 before this: FE called it but BE never implemented it). Access: class owner (tutor), an enrolled
student, or a parent of an enrolled student — others get `404` (not `403`, same convention as
`GET /sessions/:id`). `class` is `ClassDetailDto` minus `studentCount`/`sessionCount`/
`upcomingSessionCount` (derive those from `students`/`schedules` instead). `recentSession` (nullable):
the `ONGOING` session if any, else the soonest upcoming one, else the most recently past one;
`exerciseUrls` on it is hidden (`[]`) from non-owners until that session is `COMPLETED` (same gating as
`GET /sessions/:id`). `schedules` is the class's full weekly recurring schedule (`/schedules` rows).
`students` is the full roster (`ClassStudentDto[]`, includes each student's linked `parent`).
